...
public class JavaFxDemo extends Application {
					...
					new Thread(new Runnable() {
						...
						public void run() {
							try {
								...
								TODOSearch(text, (VBox) OneTab.lookup("#page_main"), bt_labelTT);
	...
	public void TODOSearch(String s, VBox outerBox, Label log) {
		...
		try {
			...
			Platform.runLater(new Runnable() {
				@Override
				public void run() {
					log.setTextFill(Color.web("#6495ED"));
			...
			Platform.runLater(new Runnable() {
				...
				public void run() {
					...
					pagination.setPageFactory((Integer pageIndex) -> createAnimalPage(pageIndex, linkstr, log));
	...
	private VBox createAnimalPage(int pageIndex, URL linkstr, Label log) {
		VBox box = new VBox();
		...
		try {
			...
			Parser parser = new Parser(apageurl);
			...
			for (Node n : dll.toNodeArray()) {
				...
				LinkTag linkTag = (LinkTag) n.getChildren().elementAt(1).getChildren().elementAt(3);
				LinkTag mlink = (LinkTag) n.getChildren().elementAt(3).getFirstChild().getFirstChild();
				String addtime = 	n.getChildren().elementAt(3).getChildren().elementAt(2).toHtml().replaceAll("<span>|&nbsp|<b>|</span>|</b>|;", "");
				String fsize = 		n.getChildren().elementAt(3).getChildren().elementAt(4).toHtml().replaceAll("<span>|&nbsp|<b>|</span>|</b>|;", "");
				String fnum = 		n.getChildren().elementAt(3).getChildren().elementAt(6).toHtml().replaceAll("<span>|&nbsp|<b>|</span>|</b>|;", "");
				String speed = 		n.getChildren().elementAt(3).getChildren().elementAt(8).toHtml().replaceAll("<span>|&nbsp|<b>|</span>|</b>|;", "");
				String Popularity = n.getChildren().elementAt(3).getChildren().elementAt(10).toHtml().replaceAll("<span>|&nbsp|<b>|</span>|</b>|;", "");
				String x = ""+addtime+" / "+fsize+" / "+fnum+" / "+speed+" / "+Popularity+"";
				String btname;
				String btn = linkTag.getLinkText().replaceAll("\r|\n", "").replaceAll("<b>|</b>|<i>|</i>", "")
						.replaceAll("  ", "");
				...
				data.add(new AMessage(false, btname, x, mlink.getLink()));
			}
				...
				if (event.getButton() == MouseButton.SECONDARY) {
					Platform.runLater(new Runnable() {
						...
						public void run() {
							...
							ObservableList<AMessage> getData = tableView.getSelectionModel().getSelectedItems();
							for(AMessage t:getData){
								
								sb.append(t.emailProperty().getValueSafe()+"\n");
							}
		...
		box.getChildren().addAll(tableView, desc);
		return box;
	}
